...
echo"<div class='abc3'><center>Update <b>Chemistry Lab</b> Attendance of your class <b>" .$sec. "</b> from Here<br><br><table cellspacing='4' cellpadding='4' border='1'>
	...
	$row = mysqli_fetch_array($result);
            ?><center>
				<form name="attendance" action="" method ="post">
					...
					<td>&nbsp Chemistry Lab &nbsp</td>
					<td>&nbsp<?php echo $row['chemistrylab'] ?>&nbsp</td>
					<td>
					<select name="attend[]">
...
$dbb = mysqli_connect('localhost', '  ','PASSWORD', '  ') or die('Error communicating to MySQL server.');
...
$attend=$_POST['attend'];
	...
	for($i=0; $i<$count; $i++){
		...
		if($attend[$i] == 'p') {
			$querye = "UPDATE students SET chemistrylab = chemistrylab + 1 WHERE roll=".$row['roll']." ";
			$sql=mysqli_query($dbb,$querye);
		} elseif($attend[$i] == 'a') {
			$querye = "UPDATE students SET chemistrylab = chemistrylab + 0 WHERE roll =".$row['roll']."";
			$sql = mysqli_query($dbb,$querye);
			}
	}unset($_POST);
}
