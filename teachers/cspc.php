	...
	$row = mysqli_fetch_array($result);
            ?><center>
				<form name="attendance" action="" method ="post">
					...
					<td>&nbsp CSPC &nbsp</td>
					<td>&nbsp<?php echo $row['cspc'] ?>&nbsp</td>
					<td>
					<select name="attend[]">
...
$attend=$_POST['attend'];
	...
	for($i=0; $i<$count; $i++){
		...
		if($attend[$i] == 'p') {
			$querye = "UPDATE students SET cspc = cspc + 1 WHERE roll=".$row['roll']." ";
			$sql=mysqli_query($dbb,$querye);
		} elseif($attend[$i] == 'a') {
			$querye = "UPDATE students SET cspc = cspc + 0 WHERE roll =".$row['roll']."";
			$sql = mysqli_query($dbb,$querye);
			}
	}unset($_POST);
}
