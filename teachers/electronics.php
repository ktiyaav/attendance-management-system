...
echo"<div class='abc3'><center>Update <b>Electronics</b> Attendance of your class <b>" .$sec. "</b> from Here<br><br><table cellspacing='4' cellpadding='4' border='1'>
	...
	$row = mysqli_fetch_array($result);
            ?><center>
				<form name="attendance" action="" method ="post">
					...
					<td>&nbsp Electronics &nbsp</td>
					<td>&nbsp<?php echo $row['electronics'] ?>&nbsp</td>
					<td>
					<select name="attend[]">
...
$dbb = mysqli_connect('localhost', ' ',' ', ' ') or die('Error communicating to MySQL server.');
...
$attend=$_POST['attend'];
	...
	for($i=0; $i<$count; $i++){
		...
		if($attend[$i] == 'p') {
			$querye = "UPDATE students SET electronics = electronics + 1 WHERE roll=".$row['roll']." ";
			$sql=mysqli_query($dbb,$querye);
		} elseif($attend[$i] == 'a') {
			$querye = "UPDATE students SET electronics = electronics + 0 WHERE roll =".$row['roll']."";
			$sql = mysqli_query($dbb,$querye);
			}
	}unset($_POST);
}
