...
echo"<div class='abc3'><center>Update <b>Maths II</b> Attendance of your class <b>" .$sec. "</b> from Here<br><br><table cellspacing='4' cellpadding='4' border='1'>
	...
	$row = mysqli_fetch_array($result);
            ?><center>
				<form name="attendance" action="" method ="post">
					...
					<td>&nbsp Maths II &nbsp</td>
					<td>&nbsp<?php echo $row['pclab'] ?>&nbsp</td>
					<td>
					<select name="attend[]">
...
$attend=$_POST['attend'];
	...
	for($i=0; $i<$count; $i++){
		...
		if($attend[$i] == 'p') {
			$querye = "UPDATE students SET maths2 = maths2 + 1 WHERE roll=".$row['roll']." ";
			$sql=mysqli_query($dbb,$querye);
		} elseif($attend[$i] == 'a') {
			$querye = "UPDATE students SET maths2 = maths2 + 0 WHERE roll =".$row['roll']."";
			$sql = mysqli_query($dbb,$querye);
			}
	}unset($_POST);
}
