...
echo"<div class='abc3'><center>Update Attendance of your class from Here<br><br><table cellspacing='4' cellpadding='4' border='1'>
	...
	$row = mysqli_fetch_array($result);
            ?><center>
				<form name="attendance" action="" method ="post">
					...
					<td>&nbsp<?php echo $ro['sub'] ?>&nbsp</td>
					<td>&nbsp<?php echo $row['mechanics'] ?>&nbsp</td>
					<td>
					<select name="attend[]">
...
$attend=$_POST['attend'];
	...
	for($i=0; $i<$count; $i++){
		...
		if($attend[$i] == 'p') {
			$querye = "UPDATE students SET mechanics = mechanics + 1 WHERE roll=".$row['roll']." ";
			$sql=mysqli_query($dbb,$querye);
		} elseif($attend[$i] == 'a') {
			$querye = "UPDATE students SET mechanics = mechanics + 0 WHERE roll =".$row['roll']."";
			$sql = mysqli_query($dbb,$querye);
			}
	}unset($_POST);
}
