...
echo"<div class='abc3'><center>Update <b>Professional Communication</b> Attendance of your class <b>" .$sec. "</b> from Here<br><br><table cellspacing='4' cellpadding='4' border='1'>
	...
	$row = mysqli_fetch_array($result);
            ?><center>
				<form name="attendance" action="" method ="post">
					...
					<td>&nbsp Professional Communication &nbsp</td>
					<td>&nbsp<?php echo $row['professionalcommunication'] ?>&nbsp</td>
					<td>
					<select name="attend[]">
...
$attend=$_POST['attend'];
	...
	for($i=0; $i<$count; $i++){
		...
		if($attend[$i] == 'p') {
			$querye = "UPDATE students SET professionalcommunication = professionalcommunication + 1 WHERE roll=".$row['roll']." ";
			$sql=mysqli_query($dbb,$querye);
		} elseif($attend[$i] == 'a') {
			$querye = "UPDATE students SET professionalcommunication = professionalcommunication + 0 WHERE roll =".$row['roll']."";
			$sql = mysqli_query($dbb,$querye);
			}
	}unset($_POST);
}
